...
export function RegionDestinationTab() {
  ...
  useEffect(() => {
    const regionDestinationPermission = permissions.find(
      (p: Permission) => p.module === "explore_destination"
    )
    setHasPermission({
      create: regionDestinationPermission?.create ?? false,
      update: regionDestinationPermission?.update ?? false,
      delete: regionDestinationPermission?.delete ?? false,
    })
  }, [])
  ...
  const fetchDestinations = async () => {
    try {
      ...
      const response = await fetch(`${API_BASE}/region?${params}`)
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        <div>
          <h2 className="text-2xl font-bold">Region Destinations</h2>
          <p className="text-sm text-muted-foreground">Manage destinations by region (North, South, East, West, etc.)</p>
        </div>
        {hasPermission.create && <Button onClick={() => router.push("/admin/explore-destination/region/new")}>
          <Plus className="h-4 w-4 mr-2" />
          Add Region Destination
        </Button>}
      </div>
      ...
      {loading ? (
        ...
        <Card>
          <CardContent className="py-8 text-center text-muted-foreground">
            No destinations found. Create your first region destination!
          </CardContent>
        </Card>
      ) : (
        <Card>
              ...
              <TableBody>
                ...
                <TableRow key={destination._id}>
                  ...
                  <TableCell className="font-medium">
                    {getRegionLabel(destination.region || "") || "N/A"}
                    {destination.state && (
                      <span className="text-muted-foreground ml-2">({destination.state})</span>
                    )}
                  </TableCell>
                  <TableCell>
                    <div className="max-w-[250px] truncate" title={destination.desc || "N/A"}>
                  ...
                  <TableCell className="text-right">
                    <div className="flex justify-end gap-2">
                      ...
                      {hasPermission.update && <Button
                        variant="ghost"
                        size="icon"
                        onClick={() => router.push(`/admin/explore-destination/region/${destination._id}`)}
                      >
      ...
      <Dialog open={isDetailsDialogOpen} onOpenChange={setIsDetailsDialogOpen}>
        <DialogContent className="max-w-[900px] sm:max-w-[900px] md:max-w-[900px] lg:max-w-[900px] max-h-[95vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>{selectedDestination && getRegionLabel(selectedDestination.region || "")}</DialogTitle>
          </DialogHeader>
          {selectedDestination && (
              ...
              {selectedDestination.image && (
                <div>
                  <img
                    ...
                    alt={getRegionLabel(selectedDestination.region || "")}
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This action cannot be undone. This will permanently delete the region destination.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>

...
interface RegionDestinationFormProps {
  initialData?: any
  isEdit?: boolean
}
...
export function RegionDestinationForm({ initialData, isEdit = false }: RegionDestinationFormProps) {
  ...
  // State mapping for each region - All 28 States and 8 Union Territories
  ...
  const getRegionLabel = () => {
    ...
    return regionMap[formData.region] || formData.region.charAt(0).toUpperCase() + formData.region.slice(1)
  }
  ...
  useEffect(() => {
    if (formData.region && !isEdit) {
      setFormData({ ...formData, state: "", placesDetails: [] })
    }
  }, [formData.region])
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.status || result.success) {
        toast({
          title: "Success",
          description: isEdit ? "Region destination updated successfully" : "Region destination added successfully",
        })
        router.push("/admin/explore-destination?tab=region")
      } else {
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <select
                ...
                onChange={(e) => setFormData({ ...formData, region: e.target.value, state: "", placesDetails: [] })}
            ...
            {formData.region && (
              <div className="space-y-2">
                ...
                <select
                  ...
                  onChange={(e) => setFormData({ ...formData, state: e.target.value, placesDetails: [] })}
                  className="w-full px-3 py-2 border rounded-md"
                  required
                >
                  <option value="">Select State</option>
                  {getStatesForRegion(formData.region).map((state) => (
              ...
              {formData.placesDetails.length === 0 ? (
                <Card>
                  <CardContent className="py-8 text-center text-muted-foreground">
                    No places added yet. Click "Add {formData.state} Place" to start.
                  </CardContent>
                </Card>
              ) : (
                <Accordion type="multiple" value={Array.from(expandedPlaces).map(String)} onValueChange={(values) => setExpandedPlaces(new Set(values.map(Number)))}>
                  {formData.placesDetails.map((place, placeIndex) => {
                    const regionLabel = formData.region.charAt(0).toUpperCase() + formData.region.slice(1)
                    return (
                          ...
                          <div className="flex items-center justify-between w-full pr-4">
                            <div className="flex items-center gap-2">
                              ...
                              <span className="font-semibold">
                                {place.placeName || `${formData.state} Place ${placeIndex + 1}`}
                              </span>
                            </div>
                          </div>
                        </AccordionTrigger>
...
// Place Detail Form Component - same as Season Destination
function PlaceDetailForm({ place, placeIndex, updatePlaceDetail, updatePlaceWithImageFiles, removePlace }: {
  place: PlaceDetail
  placeIndex: number
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="space-y-2">
          ...
          <Input
            ...
            placeholder="e.g., Shimla, Manali, Dehradun, Mussoorie"
          />
        </div>
      </div>

...
// Activity types based on category
const CATEGORY_ACTIVITY_TYPES: { [key: string]: string[] } = {
  beaches: ["Swimming", "Surfing", "Snorkeling", "Scuba Diving", "Beach Volleyball", "Beach Cricket", "Sunbathing", "Beach Sports", "Water Sports", "Beach Party"],
  mountains: ["Trekking", "Hiking", "Rock Climbing", "Mountain Biking", "Camping", "Paragliding", "Skiing", "Snowboarding", "Mountain Photography", "Nature Walk"],
  deserts: ["Desert Safari", "Camel Riding", "Sandboarding", "Dune Bashing", "Desert Camping", "Stargazing", "Desert Photography", "Quad Biking", "Hot Air Balloon", "Desert Trek"],
  forests: ["Jungle Safari", "Wildlife Spotting", "Bird Watching", "Forest Trekking", "Nature Photography", "Camping", "River Rafting", "Zip Lining", "Nature Walk", "Butterfly Watching"],
  historical: ["Heritage Walk", "Monument Visit", "Museum Tour", "Cultural Tour", "Historical Photography", "Archaeological Tour", "Guided Tour", "Audio Tour", "Cultural Experience", "Historical Reenactment"],
  religious: ["Pilgrimage", "Temple Visit", "Meditation", "Spiritual Tour", "Religious Ceremony", "Prasadam", "Aarti", "Darshan", "Religious Photography", "Spiritual Retreat"],
}

export function CategoryDestinationForm({ initialData, isEdit = false }: CategoryDestinationFormProps) {
  ...
  // Get activity types based on selected category
  const getActivityTypes = () => {
    if (!formData.category) return []
    return CATEGORY_ACTIVITY_TYPES[formData.category] || []
  }
  ...
  const addPlace = () => {
    ...
    toast({
      title: "Success",
      description: "New destination added. Fill in the details below.",
    })
  }
  ...
  const updatePlaceWithImageFiles = (index: number, placeData: any) => {
    const updatedPlaces = [...formData.placesDetails]
    updatedPlaces[index] = { ...updatedPlaces[index], ...placeData }
    setFormData({ ...formData, placesDetails: updatedPlaces })
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.status || result.success) {
        toast({
          title: "Success",
          description: isEdit ? "Category destination updated successfully" : "Category destination added successfully",
        })
        router.push("/admin/explore-destination?tab=category")
      } else {
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                ...
                <Button type="button" onClick={addPlace} size="sm">
                  ...
                  Add {formData.category.charAt(0).toUpperCase() + formData.category.slice(1)}
                </Button>
              </div>
              ...
              {formData.placesDetails.length === 0 ? (
                <Card>
                  <CardContent className="py-8 text-center text-muted-foreground">
                    No destinations added yet. Click "Add {formData.category.charAt(0).toUpperCase() + formData.category.slice(1)}" to start.
                  </CardContent>
                </Card>
              ) : (
                    ...
                    <AccordionItem key={placeIndex} value={placeIndex.toString()}>
                        ...
                        <div className="flex items-center justify-between w-full pr-4">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="font-semibold">
                              {place.placeName || `${categoryLabel} ${placeIndex + 1}`}
                            </span>
                          </div>
                        </div>
                      </AccordionTrigger>
                      <AccordionContent>
                        <PlaceDetailForm
                          ...
                          updatePlaceWithImageFiles={updatePlaceWithImageFiles}
...
// Place Detail Form Component with category-based activity types
function PlaceDetailForm({ place, placeIndex, updatePlaceDetail, updatePlaceWithImageFiles, removePlace, category, activityTypes }: {
  place: PlaceDetail
  placeIndex: number
  updatePlaceDetail: (index: number, field: keyof PlaceDetail, value: any) => void
  updatePlaceWithImageFiles: (index: number, placeData: any) => void
  ...
  return (
    <div className="space-y-6 pt-4">
      <div className="flex items-center justify-between">
        <h4 className="font-semibold">{categoryLabel} Information</h4>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="space-y-2">
          <Label>{categoryLabel} Name *</Label>
          <Input
      ...
      <div className="space-y-2">
        ...
        <Textarea
          ...
          placeholder="Weather details for this destination..."
          rows={4}
        />
      </div>
              ...
              <div className="space-y-2 md:col-span-3">
                ...
                {activityForm.image ? (
                  <img
                    ...
                    className="h-20 w-full rounded object-cover mt-2"
                  />
                ) : editingActivityIndex !== null && localPlace.activities[editingActivityIndex]?.image && typeof localPlace.activities[editingActivityIndex].image === "string" && !localPlace.activities[editingActivityIndex].imageFile ? (
                  <img

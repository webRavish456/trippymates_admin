...
import { Plus, Search, Edit, Trash2, Eye, MapPin, Star, Activity, Calendar } from "lucide-react"
...
type ExploreDestinationPermission = {
  ...
  export function CategoryDestinationTab() {
  ...
  useEffect(() => {
    const categoryDestinationPermission = permissions.find(
      (p: Permission) => p.module === "explore_destination"
    )
    setHasPermission({
      create: categoryDestinationPermission?.create ?? false,
      update: categoryDestinationPermission?.update ?? false,
      delete: categoryDestinationPermission?.delete ?? false,
    })
  }, [])
  ...
  const fetchDestinations = async () => {
    try {
      ...
      const response = await fetch(`${API_BASE}/category?${params}`)
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        <div>
          <h2 className="text-2xl font-bold">Category Destinations</h2>
          <p className="text-sm text-muted-foreground">Manage destinations by category (Beaches, Mountains, etc.)</p>
        </div>
        {hasPermission.create && <Button onClick={() => router.push("/admin/explore-destination/category/new")}>
      ...
      {loading ? (
        ...
        <Card>
          <CardContent className="py-8 text-center text-muted-foreground">
            No destinations found. Create your first category destination!
          </CardContent>
        </Card>
      ) : (
        <Card>
              ...
              <TableBody>
                ...
                <TableRow key={destination._id}>
                  <TableCell>
                    {destination.image ? (
                      <img
                        ...
                        alt={destination.category || destination.title || "Destination"}
                  ...
                  <TableCell className="font-medium capitalize">
                    {destination.category || destination.title || "N/A"}
                  </TableCell>
                  <TableCell>
                    <div className="max-w-[300px] truncate" title={destination.desc || destination.description || "N/A"}>
                  ...
                  <TableCell className="text-right">
                    <div className="flex justify-end gap-2">
                      ...
                      {hasPermission.update && <Button
                        variant="ghost"
                        size="icon"
                        onClick={() => router.push(`/admin/explore-destination/category/${destination._id}`)}
                      >
      ...
      <Dialog open={isDetailsDialogOpen} onOpenChange={setIsDetailsDialogOpen}>
        <DialogContent className="max-w-[900px] sm:max-w-[900px] md:max-w-[900px] lg:max-w-[900px] max-h-[95vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>
              {selectedDestination?.category || selectedDestination?.title
                ? (() => {
                    const name = selectedDestination.category || selectedDestination.title || ""
          ...
          {selectedDestination && (
            <div className="space-y-6">
              ...
              {selectedDestination.image && (
                <div>
                  <img
                    ...
                    alt={selectedDestination.category || selectedDestination.title || "Destination"}
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground">{selectedDestination.desc || selectedDestination.description || "No description available"}</p>
              </div>
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This action cannot be undone. This will permanently delete the category destination.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>

...
export function AdventureActivitiesTab() {
  ...
  const fetchDestinations = async () => {
    try {
      ...
      const response = await fetch(`${API_BASE}/adventure?${params}`)
  ...
  const handleDeleteDestination = async () => {
    ...
    try {
      ...
      if (result.status || result.success) {
        toast({
          title: "Success",
          description: "Adventure activity deleted successfully",
        })
        setIsDeleteDialogOpen(false)
        setSelectedDestination(null)
        fetchDestinations()
      } else {
        throw new Error(result.message || "Failed to delete adventure activity")
      }
    } catch (error: any) {
      toast({
        title: "Error",
        description: error.message || "Failed to delete adventure activity",
        variant: "destructive",
      })
    }
  }
      ...
      {loading ? (
        ...
        <Card>
          <CardContent className="py-8 text-center text-muted-foreground">
            No adventure activities found. Create your first adventure activity!
          </CardContent>
        </Card>
      ) : (
        <Card>
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This action cannot be undone. This will permanently delete the adventure activity.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>

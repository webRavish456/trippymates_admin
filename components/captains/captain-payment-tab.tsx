...
import { Calendar, CreditCard, Search, Filter } from "lucide-react"
...
const API_BASE = `${API_BASE_URL}/api/admin/captain-payment`
...
export function CaptainPaymentTab({ captainId }: { captainId: string }) {
  ...
  const handleAddPayment = async () => {
    ...
    try {
      const token = localStorage.getItem('adminToken')
      // Get latest assignment for this captain
      const assignmentResponse = await fetch(
        `${API_BASE_URL}/api/admin/captain-assignment/all?captainId=${captainId}&limit=1`,
        {
      ...
      if (!assignmentData.status || !assignmentData.data?.assignments?.length) {
        throw new Error("No assignment found for this captain")
      }

      const assignment = assignmentData.data.assignments[0]
      ...
      const response = await fetch(`${API_BASE}/add`, {
        ...
        body: JSON.stringify({
          captainId,
          bookingId: assignment.bookingId?._id || assignment.bookingId,
      ...
      <Dialog open={isPaymentDialogOpen} onOpenChange={setIsPaymentDialogOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Record a payment for this captain
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4">
